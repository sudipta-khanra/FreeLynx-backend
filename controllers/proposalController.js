...
export const getProposalsByJob = async (req, res) => {
    console.log('Request body:', req.body);  // <-- Add this line to print req.body
console.log('req.params:', req.params);

  try {
    const { jobId } = req.params;

    // Only clients or admins can view proposals
    if (req.user.role !== "client" && req.user.role !== "admin") {
      return res.status(403).json({ message: "Access denied." });
    }

    // Fetch proposals and populate freelancer details (name, email)
